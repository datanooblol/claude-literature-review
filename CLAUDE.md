...
review_literature/      ← Synthesis: proposed ideas, connections, drafts
...
4. **Keep all ideas.** Files in `review_literature/` are working drafts — accumulate them, decide which to pursue later.
...
2. Run `/propose-idea <title>` to scaffold a draft file in `review_literature/`
3. Iterate — flesh out arguments, link supporting evidence from summaries
4. Accumulate all ideas; revisit and select the strongest ones later

## What to Log
- Paper processed → row in `log/papers.md` + card in `log/overview.md`
- New idea created → file in `review_literature/`
- Significant decisions → noted inside the relevant idea file
